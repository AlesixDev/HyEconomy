...
public class AddCommand extends AbstractAsyncCommand {
    ...
    public AddCommand(@Nonnull String name, @Nonnull String description) {
        ...
        playerName = withRequiredArg("player", "The player to add money to", ArgTypes.PLAYER_REF)
        ...
        amount = withRequiredArg("amount", "The amount of money to add", ArgTypes.FLOAT);
    }
    ...
    protected CompletableFuture<Void> executeAsync(@Nonnull CommandContext commandContext) {
            try {
                ...
                final float amountToAdd = amount.get(commandContext);
                final float currentPlayerBalance = HyEconomy.getInstance().getEconomy().getBalance(targetPlayer.getUuid());

                if (currentPlayerBalance + amountToAdd < configManager.getData().balance.minimum) {
                    commandContext.sender().sendMessage(
                            Message.raw(
                                    configManager.getData().messages.min_balance_reached.replace("{player}", targetPlayer.getUsername())
                                            .replace("{balance}", String.valueOf(amountToAdd)).replace("{currency}", configManager.getData().chat.currency)
                ...
                if (currentPlayerBalance + amountToAdd > configManager.getData().balance.maximum) {
                    commandContext.sender().sendMessage(
                            Message.raw(
                ...
                HyEconomy.getInstance().getEconomy().addBalance(targetPlayer.getUuid(), amountToAdd);

                commandContext.sender().sendMessage(
                        Message.raw(
                                configManager.getData().messages.admin_add_success.replace("{player}", targetPlayer.getUsername())
                                        .replace("{balance}", String.valueOf(amountToAdd)).replace("{currency}", configManager.getData().chat.currency)
                        )
                );
                return CompletableFuture.completedFuture(null);
