...
public class SetCommand extends AbstractAsyncCommand {
    ...
    public SetCommand(@Nonnull String name, @Nonnull String description) {
        ...
        playerName = withRequiredArg("player", "The player to set money to", ArgTypes.PLAYER_REF)
        ...
        amount = withRequiredArg("amount", "The amount of money to set", ArgTypes.FLOAT);
    }
    ...
    protected CompletableFuture<Void> executeAsync(@Nonnull CommandContext commandContext) {
            try {
                    ...
                    commandContext.sender().sendMessage(
                            Message.raw(
                                    configManager.getData().messages.min_balance_reached.replace("{player}", targetPlayer.getUsername())
                                            .replace("{balance}", String.valueOf(amountToSet)).replace("{currency}", configManager.getData().chat.currency)
                ...
                if (amountToSet > configManager.getData().balance.maximum) {
                    commandContext.sender().sendMessage(
                            Message.raw(
                ...
                HyEconomy.getInstance().getEconomy().setBalance(targetPlayer.getUuid(), amountToSet);

                commandContext.sender().sendMessage(
                        Message.raw(
                                configManager.getData().messages.admin_set_success.replace("{player}", targetPlayer.getUsername())
                                        .replace("{balance}", String.valueOf(amountToSet)).replace("{currency}", configManager.getData().chat.currency)
                        )
                );
                return CompletableFuture.completedFuture(null);
